...
const logger = getModuleLogger("Valorant Helper");

export const waitForLogin = async (api: ApiClient) => {
	...
	const authenticate$ = defer(() => from(api.authenticate.fromFiles())).pipe(
		concatMap(async () => {
